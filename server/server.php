<?php 
	...
	if(isset($_POST['register'])){
        ...
        /* ********************** IS IT TAKEN ************************ */
        $query = "SELECT id FROM `users` WHERE nickname = '$username' LIMIT 1";
        ...
        if(!$missingUsername && !$missingPassword && !$missingEmail && !$notMatchingPasswords && !$invalidEmail && !$takenUsername && !$takenEmail){
            ...
            $sql = "INSERT INTO users (nickname, email, password) VALUES ('$username', '$email', '$passwordDB')";
            mysqli_query($db, $sql);
	...
	if(isset($_POST['login'])){
		...
		if(!$missingUsername && !$missingPassword){
			...
			if(mysqli_num_rows($result) == 1){
				/* ********************** LOG USER IN ************************ */
				$_SESSION['username'] = $username;
				$_SESSION['success'] = "Uspešno ste se ulogovali.";
	...
	if(isset($_GET['logout'])){
        ...
        $_SESSION['loggedout'] = "Uspešno ste se izlogovali.";
	...
	if(isset($_SESSION['username'])){
		...
		$query = "SELECT id FROM `users` WHERE nickname = '$currentUserUsername' LIMIT 1";
		$result = mysqli_query($db, $query);
